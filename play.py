...
# class for input and output interface
...
def choose_category(ui):
    chosen_category = None

    # request user to enter category until no exception
    while chosen_category is None:
        category_name = ui.input("Enter the name of your category: ").lower()
...
def computer_play(ui, category_name, chosen_category, secret_letter, secret_word):
    ...
    while user_guesses <= 5:
        player_input = ui.input("Enter your guess: ")
        print()
        ...
        # provide clues to the player after 2 fails
            ...
            ui.output(f"{Colour.blue} No, please try again {Colour.end}")
            print()
        ...
        ui.output(f"{Colour.bold} My word was {secret_word} {Colour.end}")


# Function to get the user's starting letter
def get_user_starting_letter(ui, chosen_category):
    unavailable_key = 0
...
def computer_guess_word(ui, chosen_category, user_starting_letter, used_words):
    computer_guesses = 0
    ...
    while computer_guesses <= 5:
        ...
        if len(new_words_available) < 1:
            ui.output(f"{Colour.cyan} I am out of words {Colour.end}")
            break
        else:
            word_guess = random.choice(new_words_available)
            used_words.append(word_guess)

        ui.output(f"Is your secret word {str(word_guess)} ?")
        time.sleep(2)
        user_response = ui.input().lower()
...
def user_provide_word(ui, user_starting_letter, user_words):
    while True:
        ui.output(f"{Colour.cyan} What was your word? {Colour.end}")
        user_word = ui.input().lower()
...
def user_play(ui, category_name, chosen_category, user_words=[]):
    ...
    user_starting_letter = get_user_starting_letter(ui, chosen_category)
